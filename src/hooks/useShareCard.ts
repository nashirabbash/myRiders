...
export function useShareCard() {
  ...
  const generateAndShare = async (ride: Ride): Promise<void> => {
    ...
    try {
      ...
      canvas.drawRRect(
        Skia.RRectXY(Skia.XYWHRect(48, CARD_HEIGHT - 480, CARD_WIDTH - 96, 210), 20, 20),
        cardBg
      )
      ...
      metrics.forEach((m, i) => {
        ...
        canvas.drawText(`${m.value}${m.unit}`, x, CARD_HEIGHT - 420, paint, valFont)
      ...
      paint.setColor(Skia.Color('#FFFFFF88'))
      canvas.drawText('TRACKRIDE', CARD_WIDTH - 260, CARD_HEIGHT - 60, paint, logoFont)
      paint.setColor(Skia.Color(cfg.color))
